...
ParseHeaders.py expand -s "${OUTNAME}-REV_primers-pass.fastq" --fasta \
    -f PRIMER >> $PIPELINE_LOG

# Rename primer fields
printf "  %2d: %-*s $(date +'%H:%M %D')\n" $((++STEP)) 24 "ParseHeaders rename"
ParseHeaders.py rename -s "${OUTNAME}-REV_primers-pass_reheader.fasta" --fasta \
    -f PRIMER1 PRIMER2 -k VPRIMER CPRIMER --outname $OUTNAME >> $PIPELINE_LOG

# Remove duplicate sequences
printf "  %2d: %-*s $(date +'%H:%M %D')\n" $((++STEP)) 24 "CollapseSeq"
CollapseSeq.py -s "${OUTNAME}_reheader.fasta" -n 15 \
...
SplitSeq.py group -s "${OUTNAME}_collapse-unique.fasta" -f DUPCOUNT --num 2 \
	--outname $OUTNAME >> $PIPELINE_LOG

# Create annotation table of final unique sequences
printf "  %2d: %-*s $(date +'%H:%M %D')\n" $((++STEP)) 24 "ParseHeaders table"
ParseHeaders.py table -s "${OUTNAME}_atleast-2.fasta" \
...
if $ZIP_FILES; then
    ...
    TEMP_FILES_ZIP=$(ls *.fasta | grep -vP "collapse-unique.fasta|atleast-2.fasta")
